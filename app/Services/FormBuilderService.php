...
class FormBuilderService
{
    ...
    /**
     * The entity's most recent registration-form submission, scoped to an
     * agency, whose schema and answers drive a profile's dynamic display
     * and updates.
     */
    public function registrationSubmissionFor(string $entity, int $entityId, int $agencyId): ?FormSubmission
    {
        return FormSubmission::where('entity_type', $entity)
            ->where('entity_id', $entityId)
            ->whereHas('form', fn ($q) => $q->where('agency_id', $agencyId)
                ->where('application_type', 'registration')
                ->where('user_type', $entity))
            ->with('form')
            ->latest()
            ->first();
    }
    ...
    /**
     * Build a profile's "Basic Information" block (the entity's standard
     * columns) followed by the registration form's schema-driven
     * blocks/sections/fields, each field carrying its current answer as
     * "value" - merging the entity's real column values with its
     * registration submission's dynamic answers into one tree.
     *
     * @return array<int, array<string, mixed>>
     */
    public function profileBlocks(string $entity, Model $record, ?FormSubmission $submission): array
    {
        $answers = (array) ($submission?->data ?? []);
        $schema = $submission?->form?->schema ?? ['blocks' => []];

        $basicInformationFields = collect($this->baseFields($entity))
            ...
            ->map(fn ($field) => [
                ...
                'value' => $field['name'] === 'image' ? $record->image_url : $record->{$field['name']},
            ])
            ->values();
                    ...
                    ->map(fn ($section) => [
                        ...
                        'fields' => collect($section['fields'] ?? [])
                            ->map(fn ($field) => [
                                ...
                                'options' => $field['options'] ?? null,
                                'value' => $this->profileFieldValue($field, $answers[$field['name'] ?? ''] ?? null),
                            ])
                            ->values(),
                    ])
                    ->values(),
     ...
     */
    private function profileFieldValue(array $field, mixed $value): mixed
    {
    ...
    /**
     * Validation rules for a partial update of a schema's dynamic answers:
     ...
     * @return array<string, string>
     */
    public function dynamicValidationRules(array $schema): array
    {
        $rules = [];
