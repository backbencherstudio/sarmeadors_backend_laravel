...
class CandidateDashboardTest extends TestCase
{
    ...
    public function test_candidate_dashboard_returns_screen_ready_job_feeds(): void
    {
        ...
        $response
            ...
            ->assertJsonPath('data.available_jobs.0.job_type_label', 'Short-Term Job')
            ->assertJsonPath('data.available_jobs.0.term_label', 'Short-term')
            ->assertJsonPath('data.available_jobs.0.services', ['Nanny'])
            ->assertJsonPath('data.available_jobs.0.address.street_address', '33 George Street')
            ->assertJsonPath('data.available_jobs.0.compensation.label', '$34/hr')
            ->assertJsonPath('data.available_jobs.0.status_label', 'Pending')
            ->assertJsonPath('data.available_jobs.0.actions.can_view_details', true)
